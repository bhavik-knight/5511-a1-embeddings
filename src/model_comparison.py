...
def get_similarity_scores(
    target_name: str, embeddings: dict[str, np.ndarray]
) -> dict[str, float]:
    """
...
def print_correlation_results(correlation: float, p_value: float) -> None:
    ...
    if correlation > 0.9:
        interpretation = (
            "Very strong positive correlation - rankings are nearly identical"
        )
...
def compare_models(
    target_name: str, model_a_name: str, model_b_name: str, top_n: int = 3
) -> None:
    """
